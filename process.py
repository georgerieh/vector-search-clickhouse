...
df = (
    pd.DataFrame(data, columns=["filename", "embedding"])
    .set_index("filename")
    .to_json()
)
dff = json.loads(df)
...
def process_file(output_directory, paths):
    try:
        ...
        with pq.ParquetWriter(
                os.path.join(output_directory, id + ".parquet"),
                parquet_schema,
                compression="zstd",
        ) as writer:
            i = 0
                ...
                if im_emb is None:
                    data = pd.concat(
                        [
                            meta_chunk,
                            pd.DataFrame(
                                {"image_embedding": [[0.0] * 768] * len(meta_chunk)}
                            ),
                        ],
                        axis=1,
                        copy=False,
                    ...
                    data = pd.concat(
                        [
                            meta_chunk,
                            pd.DataFrame(
                                {"image_embedding": [*im_emb[i: i + len(meta_chunk)]]}
                            ),
                        ],
                        axis=1,
                        copy=False,
                    ...
                    data["image_embedding"] = [
                        row.tolist() for row in data["image_embedding"]
                    ]
                if text_emb is None:
                    data = pd.concat(
                        [
                            data,
                            pd.DataFrame(
                                {"text_embedding": [[0.0] * 768] * len(meta_chunk)}
                            ),
                        ],
                        axis=1,
                        copy=False,
                    ...
                    data = pd.concat(
                        [
                            data,
                            pd.DataFrame(
                                {"text_embedding": [*text_emb[i: i + len(meta_chunk)]]}
                            ),
                        ],
                        axis=1,
                        copy=False,
                    ...
                    data["text_embedding"] = [
                        row.tolist() for row in data["text_embedding"]
                    ]
                i = i + len(meta_chunk)
                # you can save more columns
                data = data[list(parquet_schema.names)]
                data["caption"] = [
                    row.replace(""", ' ').replace(""", " ") for row in data["caption"]
                ]
                data["exif"] = [
                    json.loads(row) if row is not None else {} for row in data["exif"]
                ]
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        prog="process", description="Convert laion metadata and embeddings into Parquet"
    )
    ...
    meta_paths = Path(os.path.join(input_directory, "metadata")).glob(
        "metadata_*.parquet"
    )
    ids = [
        os.path.splitext(os.path.basename(metadata_file))[0].split("_")[1]
        for metadata_file in meta_paths
    ]
    ids = sorted(ids, key=lambda x: int(x))
    jobs = [
        [
            id,
            os.path.join(input_directory, "metadata", "metadata_" + id + ".parquet"),
            os.path.join(input_directory, "img_emb", "img_emb_" + id + ".npy"),
            os.path.join(input_directory, "text_emb", "text_emb_" + id + ".npy"),
        ]
        for id in ids
    ]
